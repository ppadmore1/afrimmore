-- Create a sequence for daily POS sale numbers
CREATE SEQUENCE IF NOT EXISTS pos_sale_daily_seq START 1;

-- Create function to atomically process POS sale with server-side validation
CREATE OR REPLACE FUNCTION process_pos_sale(
  ...
  p_created_by UUID,
  p_items JSONB
) RETURNS TABLE(sale_id UUID, sale_number TEXT, change_amount DECIMAL) 
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT COALESCE(MAX(SUBSTRING(sale_number FROM '\d{4}$')::INT), 0) + 1
  INTO v_product_stock -- reusing variable
  FROM pos_sales
  WHERE sale_number LIKE v_today_prefix || '%';
  
  v_sale_number := v_today_prefix || LPAD(v_product_stock::TEXT, 4, '0');
  ...
  LOOP
    ...
    SELECT unit_price, COALESCE(tax_rate, 0), stock_quantity, name
    INTO v_product_price, v_product_tax_rate, v_product_stock
    FROM products
    WHERE id = (v_item->>'product_id')::UUID;
  ...
  RETURN QUERY SELECT v_sale_id, v_sale_number, v_change_amount;
END;
$$;
